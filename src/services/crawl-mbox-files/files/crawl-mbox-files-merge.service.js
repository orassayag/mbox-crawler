const { logUtils, fileUtils, textUtils, emailUtils, validationUtils } = require('../../../utils');

class CrawlMBOXFilesMergeService {
    ...
    async appendEmailAddressesToTXTFile(data) {
        ...
        await fileUtils.appendFile({
            ...
            message: textUtils.sortAlphabetical(emailAddressesList)
        });
    }
    ...
    validateMergeRoundResults(mergeRound) {
        ...
        if (!validationUtils.isPositiveNumber(mergeRoundNumber)) {
            throw new Error(`Invalid or no mergeRoundNumber was found: ${mergeRoundNumber} (1000076)`);
        }

        if (!validationUtils.isPositiveNumber(mergeRoundEmailAddressesLimitCount)) {
            throw new Error(`Invalid or no mergeRoundEmailAddressesLimitCount was found: ${mergeRoundEmailAddressesLimitCount} (1000077)`);
        }
    }
    ...
    validateMergeResults() {
        ...
        if (!validationUtils.isExists(mergeRounds)) {
            throw new Error(`Invalid or no mergeRounds was found: ${mergeRounds} (1000075)`);
        }

        if (!validationUtils.isPositiveNumber(mergeRounds.length)) {
            throw new Error('At least 1 merge round required (1000103)');
        }
...
module.exports = CrawlMBOXFilesMergeService;