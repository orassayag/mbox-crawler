...
class CrawlService {
    ...
    async initiateCrawl() {
        const crawlResults = await this.crawlFile();
        await this.validateCrawlResults(crawlResults);
        const { linesCounter, emailAddressesCounter, createTXTFilesCounter } = crawlResults;
    ...
    async appendEmailAddressesToTXTFile(data) {
        ...
        await fileUtils.appendFile({
            ...
            message: emailAddressesList.join(',')
        });
    }
    ...
    async validateCrawlResults(crawlResults) {
        logUtils.logStatus('Validating crawl results.');
        const { linesCounter, emailAddressesCounter, createTXTFilesCounter } = crawlResults;
        if (!validationUtils.isPositiveNumber(linesCounter)) {
            throw new Error(`Invalid or no linesCounter was found: ${linesCounter} (1000008)`);
        }
        if (!validationUtils.isPositiveNumber(emailAddressesCounter)) {
            throw new Error(`Invalid or no emailAddressesCounter was found: ${emailAddressesCounter} (1000009)`);
        }
